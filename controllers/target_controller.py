...
from repository.target_repository import insert_target, get_target_by_id, delete_target, update_target, get_all_targets, \
    convert_target_to_json

targets_blueprint = Blueprint("targets", __name__)

@targets_blueprint.route("/", methods=['GET'])
def all_targets():
    return jsonify(asdict(ResponseDto(body=list(map(convert_target_to_json, get_all_targets()))))), 200
...
def target_by_id(target_id):
    return (get_target_by_id(target_id)
        .map(convert_target_to_json)
        .map(lambda target: (jsonify(asdict(ResponseDto(body=target))), 200))
        .value_or((jsonify(asdict(ResponseDto(error=f'not found target by the id {target_id}'))), 404)))
